          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            <div className="space-y-2 pt-6 border-t border-slate-100">
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                ...
                >
                  ...
                  <span className="font-bold text-sm">M-Pesa Paybill</span>
                </button>
                <button
                  type="button"
              ...
              {method === 'mpesa' ? (
                <>
                  {/* Toggle between the instant STK Push flow and manual Paybill instructions */}
                  <div className="flex gap-2 mb-4 text-xs font-bold">
                    <button
                      type="button"
                      onClick={() => setMpesaMode('stk')}
                      className={`px-3 py-1.5 rounded-full transition-all ${
                        mpesaMode === 'stk' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                      }`}
                    >
                      Pay Now
                    </button>
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full transition-all ${
                        mpesaMode === 'manual' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                      }`}
                    >
                      Pay Manually
                    </button>
                  ...
                  {mpesaMode === 'stk' ? (
                    ...
                    <>
                      <div className="bg-slate-50 rounded-xl px-5 py-1 border border-slate-200">
                        <CopyableRow label="Paybill Number" value={mpesaPaybill} />
                        <CopyableRow label="Account Number" value={mpesaAccountNumber} />
                      </div>
                      <p className="text-xs text-emerald-600 font-medium mt-3 flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                        Payments to this Paybill are recorded automatically — no need to notify us, but you're welcome to below.
                      </p>
                    </>
                  )}
                </>
            ...
            <div className="pt-6 border-t border-slate-100">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-4">
                3. Let us know you've given (optional, but helps us follow up)
              </label>

              {status === 'success' ? (
                <p className="text-center text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl p-6 text-sm">
                  Thank you! We've recorded your gift and will follow up if needed.
                </p>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="block w-full pl-14 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm text-slate-900 outline-none transition-all"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">M-Pesa Transaction Code</label>
                    <input
                      type="text"
                  ...
                  {error && (
                    <p className="text-red-600 text-center bg-red-50 border border-red-200 rounded-xl py-2.5 text-sm">
                      {error}
                    </p>
                  )}
                  ...
                  <div className="pt-4 flex items-center justify-end border-t border-slate-100">
                    ...
                    >
                      {status === 'sending' ? 'Sending...' : "I've Given — Notify the Church"}
                    </button>
                  </div>
                </form>
              )}
